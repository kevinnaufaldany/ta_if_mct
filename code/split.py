...
def export_tiles_to_folder(root_dir=ROOT_DIR, output_dir='dataset_split', grid=(2, 2)):
    """
    Export semua valid tiles (gambar + annotation JSON) ke folder output.
    
    Args:
        root_dir: Direktori sumber yang berisi gambar dan JSON annotation
        output_dir: Direktori output untuk menyimpan tiles
        grid: Grid untuk splitting (default 2x2)
    
    Returns:
        Dictionary dengan statistik export
    """
    ...
    stats = {
        ...
        'total_tiles_exported': 0,
        'images_processed': 0,
        'tiles_per_image': {}
    }
    
    for img_idx, img_name in enumerate(all_files):
        img_path = os.path.join(root_dir, img_name)
        
        # Load image
        image = cv2.imread(img_path)
        if image is None:
            print(f"[{img_idx+1}/{len(all_files)}] SKIP: Failed to load {img_name}")
            continue
        ...
        try:
            ...
            print(f"[{img_idx+1}/{len(all_files)}] SKIP: Failed to load {json_name}")
            continue
        ...
        for tile_id in range(n_tiles):
            ...
            # Process annotations untuk tile ini
            tile_shapes = []
            for shape in annotation.get('shapes', []):
                if shape['shape_type'] == 'polygon':
                    points = np.array(shape['points'], dtype=np.float32)
                    ...
                    if clipped is not None:
                        ...
                        if width >= 5 and height >= 5:
                            # Create new shape entry dengan local coordinates
                            tile_shape = {
                                'label': shape.get('label', 'cassiterite'),
            ...
            # Skip tile jika tidak ada objek
            if len(tile_shapes) == 0:
                continue
            
            # Generate output filenames
            base_name = os.path.splitext(img_name)[0]
            tile_img_name = f"{base_name}_tile{tile_id}.jpg"
            tile_json_name = f"{base_name}_tile{tile_id}.json"
            
            # Save tile image
            tile_img_path = os.path.join(output_dir, tile_img_name)
            cv2.imwrite(tile_img_path, tile_image)
            ...
            tile_annotation = {
                ...
                'shapes': tile_shapes,
                'imagePath': tile_img_name,
                'imageData': None,  # Tidak menyimpan base64 image data
                'imageHeight': tile_image.shape[0],
                'imageWidth': tile_image.shape[1]
            }
            
            # Save tile JSON
            tile_json_path = os.path.join(output_dir, tile_json_name)
            with open(tile_json_path, 'w', encoding='utf-8') as f:
                json.dump(tile_annotation, f, indent=2, ensure_ascii=False)
            
            tiles_exported_for_image += 1
            stats['total_tiles_exported'] += 1
        
        stats['images_processed'] += 1
        stats['tiles_per_image'][img_name] = tiles_exported_for_image
        
        print(f"[{img_idx+1}/{len(all_files)}] {img_name}: {tiles_exported_for_image} tiles exported")
    
    print("="*80)
    print("EXPORT SUMMARY:")
    print(f"  Images processed: {stats['images_processed']}/{stats['total_images']}")
    print(f"  Total tiles exported: {stats['total_tiles_exported']}")
    print(f"  Average tiles per image: {stats['total_tiles_exported']/max(stats['images_processed'], 1):.2f}")
    print(f"  Output directory: {output_dir}")
    print("="*80)
...
if __name__ == "__main__":
    # ========================================================================
    # OPTION 1: Export tiles ke folder dataset_split
    # ========================================================================
    print("\n" + "="*80)
    print("EXPORTING TILES TO FOLDER")
    print("="*80)
    
    export_stats = export_tiles_to_folder(
        root_dir=ROOT_DIR,
    ...
    print("\n" + "="*80)
    print("TESTING DATASET WITH EXPORTED TILES")
    print("="*80)
    
    # ========================================================================
    # OPTION 2: Prepare K-Fold datasets (original flow)
    # ========================================================================
    # Prepare datasets
